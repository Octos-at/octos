...
/* ========================================================================== 
 Parallax + EmailJS Submit Handler (GitHub-ready)
 ========================================================================== */
(function () {
  'use strict';
  ...
  window.addEventListener('resize', function () {
    ...
    else { parallaxBgs.forEach(function (bg) { bg.style.transform = 'translate3d(0,0,0)'; }); }
  });
  ...
  // --- Mobile Toggle ---
  if (navToggle) {
    navToggle.addEventListener('click', function () { navToggle.classList.toggle('active'); navLinks.classList.toggle('open'); });
  }
  ...
  function updateActiveLink() {
    ...
    sections.forEach(function (section) {
      var top = section.offsetTop; var height = section.offsetHeight; var id = section.getAttribute('id');
  ...
  function checkReveal() {
    var windowHeight = window.innerHeight; var revealPoint = 120;
  ...
  if (contactForm) {
    contactForm.addEventListener('submit', function (e) {
      ...
      if (typeof emailjs === 'undefined') {
        ...
        showToast('Unable to send right now. Please try again later.');
        return;
      }

      var params = { name: name, email: email, message: message, to_email: 'info@octos.at' };

      emailjs.send('service_3kflnuh', 'template_un57291', params)
        .then(function(){ showToast('Thank you! Your message has been sent.'); contactForm.reset(); })
        .catch(function(err){ console.error('EmailJS error:', err); showToast('Sorry, something went wrong. Please try again later.'); });
    });
  }
})();
