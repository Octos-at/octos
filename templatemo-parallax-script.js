...
/* ========================================================================== 
 Parallax + EmailJS Submit Handler (GitHub-ready) — FIXED
 ========================================================================== */
(function () {
  'use strict';
...
if (contactForm) {
  contactForm.addEventListener('submit', function (e) {
    ...
    if (!emailjsReady()) {
      console.error('EmailJS SDK not loaded or not initialized');
      showToast('Unable to send right now. Please try again later.');
      return;
    }

    // IMPORTANT: These must match your EmailJS dashboard
    var SERVICE_ID = 'service_3kflnuh';
    var TEMPLATE_ID = 'template_un57291';
    var PUBLIC_KEY = 'pEt0L5IFostMcoXn'; // same as emailjs.init
    ...
    emailjs.send(SERVICE_ID, TEMPLATE_ID, params, PUBLIC_KEY)
      ...
      .catch(function (err) {
        ...
        var msg = (err && (err.text || err.message)) ? String(err.text || err.message) : 'Sorry, something went wrong. Please try again later.';
        showToast(msg);
      });
  });
}
